              ...
              <tr>
                <td colSpan="6" className="px-6 py-20 text-center">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm text-gray-500 font-medium">Belum ada data customer</p>
                    <p className="text-xs text-gray-400">Gunakan tombol "Tambah Customer" untuk memulai</p>
                  </div>
                </td>
              </tr>
            )}
      ...
      {meta.total_pages > 1 && (
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          <p className="text-xs text-gray-500">
            Menampilkan <span className="font-bold">{(meta.page - 1) * meta.limit + 1}</span> - <span className="font-bold">{Math.min(meta.page * meta.limit, meta.total)}</span> dari <span className="font-bold">{meta.total}</span> data
          </p>
            ...
            <div className="flex items-center gap-1">
              {[...Array(meta.total_pages)].map((_, i) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-xs font-bold transition-all ${meta.page === i + 1 ? 'bg-blue-900 text-white shadow-md shadow-blue-900/20' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  {i + 1}
                </button>
              ))}
