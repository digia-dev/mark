...
const LeadForm = ({ isOpen, onClose, onSubmit, initialData, isLoading, isEmbedded = false }) => {
  ...
  const handleFormSubmit = (data) => {
    onSubmit(data);
    if (!initialData && !isEmbedded) reset();
  };

  const formContent = (
    <div className={`flex flex-col h-full ${isEmbedded ? '' : 'max-h-[90vh]'}`}>
      {/* Header */}
      {!isEmbedded && (
      ...
      {/* Form Content */}
      <form onSubmit={handleSubmit(handleFormSubmit)} className={`${isEmbedded ? 'p-0' : 'flex-1 overflow-y-auto p-8 scrollbar-hide'}`}>
        ...
        {/* Footer Actions */}
        <div className={`mt-10 flex gap-4 ${isEmbedded ? 'justify-end' : ''}`}>
          <button 
            type="button"
            onClick={onClose}
            className={`${isEmbedded ? 'px-8' : 'flex-1'} py-4 bg-white border border-gray-100 text-gray-700 rounded-2xl text-sm font-black hover:bg-gray-50 transition-all`}
          >
            Batal
          </button>
          <button 
            type="submit"
            disabled={isLoading}
            className={`${isEmbedded ? 'px-12' : 'flex-2'} py-4 bg-blue-900 text-white rounded-2xl text-sm font-black hover:bg-blue-800 transition-all shadow-xl shadow-blue-900/20 flex items-center justify-center gap-2`}
          >
            {isLoading ? (
