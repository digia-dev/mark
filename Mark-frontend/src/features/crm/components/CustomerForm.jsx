...
const CustomerForm = ({ isOpen, onClose, onSubmit, initialData, isLoading, isEmbedded = false }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const content = (
    <div className={`${isEmbedded ? '' : 'bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200'}`}>
      {/* Header */}
      {!isEmbedded && (
      ...
      {/* Form Body */}
      <form onSubmit={handleSubmit(onSubmit)} className={isEmbedded ? 'p-0' : 'p-6'}>
        ...
        <div className="mt-8 pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="px-8 py-2.5 bg-blue-900 text-white text-sm font-bold rounded-xl hover:bg-blue-800 transition-all shadow-lg shadow-blue-900/20 flex items-center gap-2 disabled:opacity-50 active:scale-95"
          >
            {isLoading ? (
